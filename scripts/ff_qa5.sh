#!/usr/bin/env bash
# FF QA round 5 — verbose preset debug + map ch6 + play ch6
set -uo pipefail
...
$AB open "http://localhost:3000/?nofpsguard=1&v=7" >/dev/null
sleep 1.5
echo "--- preset ---"
$AB eval "
(() => {
  ...
  return 'preset-ok levels=' + Object.keys(s.levels).length;
})()"
...
sleep 2.2
$AB screenshot scripts/ff5_map_ch6.png >/dev/null

echo "--- play ch6 lv1 ---"
$AB eval "[...document.querySelectorAll('.map-node')].find(b=>b.innerText.trim()==='۱')?.click()" >/dev/null
sleep 2.6
$AB screenshot scripts/ff5_play_ch6.png >/dev/null
echo "--- music requests ---"
